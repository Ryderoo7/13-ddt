...
def create_tables(conn):
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS users
                 (username TEXT PRIMARY KEY, password TEXT, weight TEXT)''')
...
def signupbutton():
    ...
    def register_user():
        ...
        if existing_user:
            ...
            cursor.execute(
                "INSERT INTO users (username, password, weight) VALUES (?, ?, ?)",
                (username,
                 password,
                 weight))
            conn.commit()
    ...
    # explains why we use weight as a stored value.
    def show_weight_info(event):
        """
        Display a pop-up message explaining why the user needs to enter their weight.
        """
        weight_info = (
            "Entering your weight helps the app to track your progress\n"
            "It also allows you to track your progress over time.\n"
            "By helping your refrain from seeing your original weight \n"
            "stopping you getting stressed over your progress\n"
            "which will produce cortisol, ultimately harming your weight goals"
        )
        messagebox.showinfo("Why Enter Weight?", weight_info)

    def hide_weight_info(event):
        """
        Hide the pop-up message.
        """
        pass  # This function is for the Leave event.

    weight_info_label = Label(
        signup_window,
        text="Why enter your weight?",
        fg="black",
        cursor="question_arrow")
    weight_info_label.place(relx=0.5, rely=0.88, anchor="center")
    weight_info_label.bind("<Enter>", show_weight_info)
    weight_info_label.bind("<Leave>", hide_weight_info)
    # Functions to remove and insert insterts based on focus in the entry boxes.
    def entry_username_focus(event):
    ...
    # Functions to remove and insert insterts based on focus in the entry boxes.
    def weight_singup_focus(event):
        if weight_signup.get() == "Weight":
            weight_signup.delete(0, END)
            weight_signup.configure(foreground="black")
    # Functions to remove and insert insterts based on focus in the entry boxes.
    def weight_signup_leave(event):
        if weight_signup.get() == "":
            weight_signup.insert(0, "Weight")
    ...
    # Function to validate weight input wheher it's a valid integer.
    def validate_weight_input():
        if not validate_integer_input(weight_signup.get()):
            messagebox.showerror("Error", "Please only type integers")
            return False
        return True
    ...
    weight_signup = Entry(frame_signup, bg="#da3422",
                          relief="flat", foreground="black")
    weight_signup.insert(0, "Weight")
    weight_signup.bind('<FocusIn>', weight_singup_focus)
    weight_signup.bind('<FocusOut>', weight_signup_leave)
    weight_signup.pack(pady=10)
    ...
    button_submit.configure(command=lambda: register_user() if validate_weight_input(
    ) else messagebox.showerror("Error", "Please only type integers"))
