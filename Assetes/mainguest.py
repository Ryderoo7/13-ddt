...
# Function to calculate the estimated daily calorie intake.
def calculate_calories():
    ...
    # Check if any criteria is missed.
    ...
    if gender == "Male":
        ...
        return
    # Response dictionary for activity levels.
    ...
    calories_window.resizable(width=True, height=True)
    ...
    # Create and place the red circle image.
    Redcircle = Label(calories_window, image=logo_image, bg="#ffc266")
    Redcircle.image = logo_image
    Redcircle.place(relx=0.5, rely=0.2, anchor="center")
    ...
    # Create and place the calculated calories label.
...
# Function to toggle the maintain weight option.
def maintain_weight_toggle():
    """
...
# Function to toggle the lose weight option.
def lose_weight_toggle():
    """
...
def age_entry_focus(event):
    """
    Triggered when the age entry widget gains focus. If the entry 
    contains the placeholder Age
    and it hasn't been changed yet, this function
    clears the entry and changes its text color to black.
    """
...
def age_entry_leave(event):
    """
    Triggered when the age entry widget loses focus. If the entry 
    is empty and the placeholder
...
def weight_entry_focus(event):
    ...
    If the entry contains the placeholder
    Weight and it hasn't been changed yet, this function clears 
...
def height_entry_focus(event):
    """
    Triggered when the height entry widget gains focus. If the entry
    contains the placeholder Heigh
...
# multiplier dictionary for activity levels
activity_multiplier = {
...
label_login = Label(
    root,
    text="Your logged in as Guest ",
    font=("Arial", 18, "bold"),
    bg="#ffc266",
    fg="black",
)
label_login.place(relx=0.515, rely=0.03, anchor="center")
